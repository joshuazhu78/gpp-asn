...
var FreqBandInformationknownImplementations = []isFreqBandInformation_FreqBandInformation{
	&FreqBandInformation_BandInformationEutra{},
	&FreqBandInformation_BandInformationNr{},
}

func (c *FreqBandInformation) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range FreqBandInformationknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.FreqBandInformation = target.Elem().Interface().(isFreqBandInformation_FreqBandInformation)
			return nil
		}
	}
	return nil
...
func (c FreqBandInformation) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.FreqBandInformation).String(),
		Value: c.FreqBandInformation,
	})
}

var _ json.Unmarshaler = (*FreqBandInformation)(nil)
