...
var C1UlCcchMessageTypeknownImplementations = []isC1UlCCchMessageType_C1UlCcchMessageType{
...
func (c *C1UlCCchMessageType) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range C1UlCcchMessageTypeknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.C1UlCcchMessageType = target.Elem().Interface().(isC1UlCCchMessageType_C1UlCcchMessageType)
			return nil
		}
	}
	return nil
...
func (c C1UlCCchMessageType) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.C1UlCcchMessageType).String(),
		Value: c.C1UlCcchMessageType,
	})
}

var _ json.Unmarshaler = (*C1UlCCchMessageType)(nil)
