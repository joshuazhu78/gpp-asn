...
var ReportTypeReportConfigNrknownImplementations = []isReportTypeReportConfigNr_ReportTypeReportConfigNr{
	&ReportTypeReportConfigNr_Periodical{},
}

func (c *ReportTypeReportConfigNr) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range ReportTypeReportConfigNrknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.ReportTypeReportConfigNr = target.Elem().Interface().(isReportTypeReportConfigNr_ReportTypeReportConfigNr)
			return nil
		}
	}
	return nil
...
func (c ReportTypeReportConfigNr) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.ReportTypeReportConfigNr).String(),
		Value: c.ReportTypeReportConfigNr,
	})
}

var _ json.Unmarshaler = (*ReportTypeReportConfigNr)(nil)
