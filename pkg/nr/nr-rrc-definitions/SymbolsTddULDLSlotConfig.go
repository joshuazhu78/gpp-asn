...
var SymbolsTddUlDlSlotConfigknownImplementations = []isSymbolsTddULDLSlotConfig_SymbolsTddUlDlSlotConfig{
	&SymbolsTddULDLSlotConfig_AllDownlink{},
	&SymbolsTddULDLSlotConfig_AllUplink{},
	&SymbolsTddULDLSlotConfig_Explicit{},
}

func (c *SymbolsTddULDLSlotConfig) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range SymbolsTddUlDlSlotConfigknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.SymbolsTddUlDlSlotConfig = target.Elem().Interface().(isSymbolsTddULDLSlotConfig_SymbolsTddUlDlSlotConfig)
			return nil
		}
	}
	return nil
...
func (c SymbolsTddULDLSlotConfig) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.SymbolsTddUlDlSlotConfig).String(),
		Value: c.SymbolsTddUlDlSlotConfig,
	})
}

var _ json.Unmarshaler = (*SymbolsTddULDLSlotConfig)(nil)
