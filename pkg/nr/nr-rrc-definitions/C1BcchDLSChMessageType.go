...
var C1BcchDlSchMessageTypeknownImplementations = []isC1BcchDLSChMessageType_C1BcchDlSchMessageType{
	&C1BcchDLSChMessageType_SystemInformation{},
	&C1BcchDLSChMessageType_SystemInformationBlockType1{},
}

func (c *C1BcchDLSChMessageType) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range C1BcchDlSchMessageTypeknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.C1BcchDlSchMessageType = target.Elem().Interface().(isC1BcchDLSChMessageType_C1BcchDlSchMessageType)
			return nil
		}
	}
	return nil
...
func (c C1BcchDLSChMessageType) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.C1BcchDlSchMessageType).String(),
		Value: c.C1BcchDlSchMessageType,
	})
}

var _ json.Unmarshaler = (*C1BcchDLSChMessageType)(nil)
