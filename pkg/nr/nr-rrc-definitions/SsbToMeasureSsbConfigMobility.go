...
var SsbToMeasureSsbConfigMobilityknownImplementations = []isSsbToMeasureSsbConfigMobility_SsbToMeasureSsbConfigMobility{
	&SsbToMeasureSsbConfigMobility_Release{},
	&SsbToMeasureSsbConfigMobility_Setup{},
}

func (c *SsbToMeasureSsbConfigMobility) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range SsbToMeasureSsbConfigMobilityknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.SsbToMeasureSsbConfigMobility = target.Elem().Interface().(isSsbToMeasureSsbConfigMobility_SsbToMeasureSsbConfigMobility)
			return nil
		}
	}
	return nil
...
func (c SsbToMeasureSsbConfigMobility) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.SsbToMeasureSsbConfigMobility).String(),
		Value: c.SsbToMeasureSsbConfigMobility,
	})
}

var _ json.Unmarshaler = (*SsbToMeasureSsbConfigMobility)(nil)
