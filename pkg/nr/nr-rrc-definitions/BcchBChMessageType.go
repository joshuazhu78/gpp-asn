...
var BcchBchMessageTypeknownImplementations = []isBcchBChMessageType_BcchBchMessageType{
	&BcchBChMessageType_Mib{},
	&BcchBChMessageType_MessageClassExtension{},
}

func (c *BcchBChMessageType) UnmarshalJSON(bytes []byte) error {
	var data struct {
		Type  string
	...
	for _, knownImplementation := range BcchBchMessageTypeknownImplementations {
		...
		if knownType.String() == data.Type {
			...
			c.BcchBchMessageType = target.Elem().Interface().(isBcchBChMessageType_BcchBchMessageType)
			return nil
		}
	}
	return nil
...
func (c BcchBChMessageType) MarshalJSON() ([]byte, error) {
	// Marshal to type and actual data to handle unmarshaling to specific interface type
	return json.Marshal(struct {
		Type  string
		Value any
	}{
		Type:  reflect.TypeOf(c.BcchBchMessageType).String(),
		Value: c.BcchBchMessageType,
	})
}

var _ json.Unmarshaler = (*BcchBChMessageType)(nil)
